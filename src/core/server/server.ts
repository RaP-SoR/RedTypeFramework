...
on("onResourceStart", (resourceName: string) => {
  if (resourceName === GetCurrentResourceName()) {
    ...
    server.start()
      ...
      .catch(err => logError("Fehler beim Starten des RedType Framework Servers", err));
  }
});
...
on("onResourceStop", (resourceName: string) => {
  if (resourceName === GetCurrentResourceName()) {
    ...
    server.stop()
      .then(() => logInfo("RedType Framework Server erfolgreich heruntergefahren"))
      .catch(err => logError("Fehler beim Herunterfahren des RedType Framework Servers", err));
  }
});
