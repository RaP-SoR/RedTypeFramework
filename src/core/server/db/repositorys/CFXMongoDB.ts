...
export class CFXMongoDB<T extends IBaseModel> implements IRepository<T> {
  ...
  public async findById(id: string): Promise<T | null> {
    try {
      const result = await exports['cfx-mongodb'].findById(this.collectionName, id);
  ...
  public async find(filter: Partial<T>): Promise<T[]> {
    try {
      ...
      const results = await exports['cfx-mongodb'].find(this.collectionName, mongoFilter);
  ...
  public async findOne(filter: Partial<T>): Promise<T | null> {
    try {
      const mongoFilter = this.prepareFilter(filter);
      const result = await exports['cfx-mongodb'].findOne(this.collectionName, mongoFilter);
  ...
  public async create(data: Omit<T, "id" | "createdAt" | "updatedAt">): Promise<T> {
    try {
      ...
      const result = await exports['cfx-mongodb'].insertOne(this.collectionName, doc);
  ...
  public async update(id: string, data: Partial<T>): Promise<T | null> {
    try {
      ...
      const result = await exports['cfx-mongodb'].findOneAndUpdate(
        this.collectionName,
        { _id: id },
        updateDoc,
  ...
  public async delete(id: string): Promise<boolean> {
    try {
      const result = await exports['cfx-mongodb'].deleteOne(this.collectionName, { _id: id });
  ...
  public async count(filter: Partial<T> = {}): Promise<number> {
    try {
      const mongoFilter = this.prepareFilter(filter);
      return await exports['cfx-mongodb'].countDocuments(this.collectionName, mongoFilter);
    } catch (error) {
