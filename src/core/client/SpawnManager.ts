...
export class SpawnManager {
  ...
  constructor() {
    if (typeof PlayerPedId === "undefined") {
      logInfo(
        "SpawnManager: Running on server side, skipping client initialization"
      );
      return;
    }
  ...
  private setupEventHandlers(): void {
    if (typeof PlayerPedId === "undefined") return;
  ...
  private async spawnPlayer(spawnData: SpawnData): Promise<void> {
    if (typeof PlayerPedId === "undefined") {
      logInfo("SpawnManager: Cannot spawn on server side");
      return;
    }
    if (this.isSpawning) {
    ...
    try {
      ...
      logInfo(
        `SpawnManager: Player activated - PedID: ${finalPed}, Visible: ${IsEntityVisible(
          finalPed
        )}, Alpha: ${GetEntityAlpha(finalPed)}`
      );
  ...
  private async setPlayerModel(model: string): Promise<void> {
    ...
    if (newPed && newPed !== -1) {
      ...
      logInfo(
        `SpawnManager: Player model set successfully - PedID: ${newPed}, Model: ${GetEntityModel(
          newPed
        )}`
      );
    } else {
      console.error(
        "SpawnManager: Failed to get new player ped after model change"
      );
    }
  }
  ...
  private requestInitialSpawn(): void {
    if (typeof PlayerPedId === "undefined") {
      logInfo("SpawnManager: Cannot request spawn from server side");
      return;
    }
...
let spawnManagerInstance: SpawnManager | null = null;
if (typeof PlayerPedId !== "undefined") {
  spawnManagerInstance = SpawnManager.getInstance();
}
